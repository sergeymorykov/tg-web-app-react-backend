...
def get_label():
    ...
    knn = KNeighborsClassifier(n_neighbors=3).fit(X_umap, Y_pred)
    user_cluster = knn.predict([data[id_user]])[0]

    same_cluster_ids = [idx for idx, label in enumerate(Y_pred) if label == user_cluster and idx != id_user]
    fullnames = [user['fullname'] for user in data if user['id_user'] in same_cluster_ids]
    same_cluster_fullnames = ', '.join(fullnames)
    
    return user_cluster, same_cluster_fullnames
